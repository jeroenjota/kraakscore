<template>
  <div class="rounded-lg border border-blue-200 bg-white/90 p-3">
    <div class="mb-2 flex flex-wrap items-center justify-between gap-3">
      ...
      <div
        class="inline-flex gap-2 rounded-lg border border-sky-300 bg-transparent p-1 shadow-sm">
        <button
          type="button"
          v-tooltip="'Tussenstand zichtbaar'"
          class="min-w-12.5 relative rounded-lg border border-sky-600 px-1 py-0.5 text-xs font-medium transition"
          :class="
        ...
        <button
          ...
          class="min-w-12.5 relative rounded-full border border-sky-600 px-1 py-0.5 text-xs font-medium transition"
          :class="
            submitMode === 'final'
              ? 'bg-sky-600 text-white'
              : 'text-sky-900 hover:bg-sky-50'
      ...
      <button
        v-tooltip="'Links tonen/verbergen'"
        type="button"
        role="switch"
        ...
        <span
          ...
          <span
            ...
            :class="
              selectedRoundVisible ? 'translate-x-4' : 'translate-x-0.5'
            " />
        </span>
        <span>{{ selectedRoundVisible ? "Links" : "Links tonen" }}</span>
      </button>
      <label
        v-if="selectedRoundVisible"
    ...
    <div class="max-h-64 space-y-2 overflow-y-auto pr-1">
      <template v-if="hasGroupTournament && selectedRoundVisible">
        <div
          v-if="selectedGroupRoundEntries"
          class="rounded border border-gray-200 p-2">
          <div>
            <div
              v-for="entry in selectedGroupRoundEntries.groupA"
              :key="entry.id"
              class="mb-2 rounded border border-gray-100 p-2 last:mb-0">
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="text-sm">
                  <span class="text-blue-800">{{ entry.label }}</span>
                  <strong class="text-gray-900">
                    - {{ entry.teamL }} vs {{ entry.teamR }}</strong
                  >
                </div>
          ...
          <div class="mt-2 border-t border-gray-700 pt-0">
            <div
              v-for="entry in selectedGroupRoundEntries.groupB"
              :key="entry.id"
              class="mb-2 rounded border border-gray-100 p-2 last:mb-0">
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="text-sm">
                ...
                <div class="flex items-center gap-2" v-tooltip="'Open QR-code en scan met telefoon'">
                  <button
                    v-tooltip="'Open QR-code en scan met telefoon'"
                    class="rounded bg-sky-600 px-2 py-1 text-xs text-white"
                    @click="openQr(entry)">
                  ...
                  <button
                    class="rounded bg-gray-100 px-2 py-1 text-xs"
                    v-tooltip="'Kopieer link naar klembord'"
                    @click="copyLink(entry.url)">
                    Kopieer link
                  </button>
                </div>
        ...
        <div
          v-else-if="selectedFinalEntries.length > 0"
          class="rounded border border-gray-200 p-2">
          <div
            v-for="entry in selectedFinalEntries"
            :key="entry.id"
            class="mb-2 rounded border border-gray-100 p-2 last:mb-0">
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-sm">
      ...
      <div
        v-else-if="displayedFormEntries.length > 0"
        v-for="entry in displayedFormEntries"
        :key="entry.id"
        class="rounded border border-gray-200 p-2">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div class="text-sm">
            ...
            <span class="text-gray-900">
              - {{ entry.teamL }} vs {{ entry.teamR }}</span
            >
          </div>
          <div class="flex items-center gap-2">
            <button
      ...
      <div
        ...
        <div
          ...
          <p class="mt-2 text-center text-xs text-gray-500">
            Scan om scoreformulier te openen
          </p>
        </div>
      </div>
    </div>
...
const hasGroupTournament = computed(
  () => Array.isArray(props.groupMatches) && props.groupMatches.length > 0,
);
      ...
      groupRounds.forEach((roundMatches, roundIndex) => {
        roundMatches.forEach((match, matchIndex) => {
          entries.push({
            ...
            label: `Grp ${groupName} - Rnd ${roundIndex + 1} - Tafel ${
              match.tafel
            }`,
...
const selectedRoundEntries = computed(() => {
  ...
  return formEntries.value.filter(
    (entry) => entry.roundKey === selectedRound.value,
  );
});
  ...
  (options) => {
    ...
    options.forEach((option) => {
      nextVisibility[option.value] =
        roundVisibility.value[option.value] !== false;
    });
