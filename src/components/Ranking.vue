...
<template>
  <div
    ...
    <div class="titelregel flex justify-between items-center bg-blue-800">
      <h1 class="text-xl font-bold mb-2 bg-blue-800">
        Ranking na {{ toernooien.length }} toernooien in
        {{ getSemester(vanaf, true) }}
      </h1>
      <button
        @click="maakRankingPdf"
        class="bg-blue-500 text-white px-2 rounded mt-1 mr-2"
        v-tooltip="'Afdrukken naar PDF'"
        style="margin-left: 2px; width: auto; height: 30px; font-size: 0.9em">
        <printer class="h-6 w-6 text-white" />
      </button>
    </div>
    <table
      class="border-2 border-collapse border-gray-800 text-sm rounded"
      id="rankingTable">
      <thead>
        <tr class="border-2">
          ...
          <th
            class="p-0 border text-center font-normal w-8"
      ...
      <tbody class="border border-collapse border-gray-800">
        <tr
          ...
          <td class="p-1 border text-center text-xs">
            <span v-if="gespeeld(index) !== 0">{{ gespeeld(index) }}</span>
            <span v-else>-</span>
          </td>
          <td
            class="p-1 text-center border text-xs"
            v-for="res in r.scores"
            :key="res.datum">
            <span v-if="res.punten !== 0">{{ res.punten }}</span>
            <span v-else>-</span>
          </td>
          <td class="p-1 border text-center font-bold text-blue-800">
            <span v-if="r.totaal !== 0">{{ r.totaal }}</span>
            <span v-else>-</span>
          </td>
        </tr>
      </tbody>
    ...
    <p class="text-white text-center text-sm">
      Punten: 1e = 12; 2e = 9 ; 3e = 6; 4e = 3; 5e of meer: 1 | Beste 6 scores
      tellen
    </p>
  </div>
</template>
...
const toernooien = computed(() => {
  return props.toernooien
    .filter(
      (t) =>
        new Date(t.datum) >= new Date(props.vanaf) &&
        new Date(t.datum) <= new Date(props.tot),
    )
