  ...
  <table :id="setID(group)">
    <thead>
      <tr>
        ...
        <th style="width: 25%; text-align: right" class="border px-2">
          Punten
        </th>
        <th
          v-if="!group"
    ...
    <tbody>
      <tr v-for="(team, i) in standings" :key="team.name">
        <td style="width: 5%; text-align: center" class="border px-2">
          {{ i + 1 }}
        </td>
        <td style="width: 60%; text-align: left" class="border px-2">
          {{ team.name }}
        </td>
        <td
          style="width: 5%; text-align: center"
          class="border px-2 text-center">
          {{ team.played }}
        </td>
        <td
